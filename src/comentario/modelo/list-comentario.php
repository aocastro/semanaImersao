<?php
    ...
    if($conexao){
        ...
        $sql = "SELECT idcomentario, date_format(data_comentario,'%d/%m/%Y %H:%i:%s') as data_comentario, comentario FROM CATEGORIAS WHERE 1=1 ";
        ...
        if( !empty( $filtro ) ){
            ...
            $sql .= " AND (idcomentario LIKE '$filtro%' ";
            $sql .= " OR data_comentario LIKE '$filtro%') ";
            $sql .= " OR comentario LIKE '$filtro%') ";
