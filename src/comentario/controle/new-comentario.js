...
$(document).ready(function() {
    ...
    $('.btn-new').click(function(e) {
        ...
        $('.modal-title').append('<h4 class="text-danger">Novo Comentário</h4>')

        // Aqui iremos criar uma constante para receber a data e hora local da criação da notícia
        const dataComentario = new Date().toLocaleString()

        // Aqui será inserido o formulário do arquivo que criamos anteriomente na pasta visão onde ficam nossos HTML
        $('.modal-body').load('src/comentario/visao/form-comentario.html', function() {
            $('#data_comentario').val(dataComentario)
            $('#data_comentario').attr('readonly', 'true')
        })
